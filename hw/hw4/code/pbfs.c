...
int pbfs(struct Graph *graph, int s, int proc_size, int rank)
{
    ...
    while (!globalIsEmpty)
    {
        if(!localIsEmpty){
            ...
            while (temp != NULL)
            {
                ...
                if (d[temp->vertex] == -1)
                {
                    ...
                    push(NS, temp->vertex);
                    d[temp->vertex] = level;
                    printf("node is: %d,  level is: %d\n",temp->vertex, d[temp->vertex]);
                    // printf("visited vertex: %d at level: %d\n", temp->vertex, level);
        ...
        // new!!
                    ...
                    int dest[100*proc_size]; // this will receive nodes from all procs
                    ...
                    int ns_size = NS->top;  // size of this stack

                    // printf("hello, i am ns_size: %dd\n", ns_size);
                    int ns_sizes[proc_size]; // to store stack size for
                    ...
                    MPI_Allgather(NS->array, 100, MPI_INT, dest, 100, MPI_INT, MPI_COMM_WORLD);
                    res = MPI_Barrier (MPI_COMM_WORLD); // procs wait
                    MPI_Allgather(&ns_size, 1, MPI_INT, ns_sizes, 1, MPI_INT, MPI_COMM_WORLD);
        ...
        // printf("ns final size aught be > 0: %d\n", NS->top);
        while (!isEmpty(NS))
            {
                ...
                int t = pop(NS);
                // printf("t: %d, proc_size: %d, rank: %d\n", t, proc_size, rank);
                if( (t % proc_size) == rank){
                    // printf("node is: %d,  level is: %d\n",t, d[t]);
                    // int t_level = d[u] + 1;
                    // printf("visited node %d at level %d\n", t, t_level);
                    push(FS, t);
                }
